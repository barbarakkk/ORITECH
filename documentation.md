...
2. [Players Management](#2-players-management)
   - [2.1 All Players](#21-all-players)
   - [2.2 KYC Queue](#22-kyc-queue)
   - [2.3 Archived Players](#23-archived-players)
3. [VIP Management](#3-vip-management)
   - [3.1 VIP Players](#31-vip-players)
   - [3.2 VIP Player Profile — Tab-by-Tab Purpose](#32-vip-player-profile--tab-by-tab-purpose)
4. [Games](#4-games)
   - [4.1 All Games](#41-all-games)
   - [4.2 Game Analytics](#42-game-analytics)
   - [4.3 Providers](#43-providers)
   - [4.4 Categories](#44-categories)
   - [4.5 Game Blocks](#45-game-blocks)
5. [Engagement](#5-engagement)
   - [5.1 Bonuses](#51-bonuses)
   - [5.2 Cashback](#52-cashback)
   - [5.3 Rakeback](#53-rakeback)
   - [5.4 Bonus Wheel](#54-bonus-wheel)
   - [5.5 Leaderboards](#55-leaderboards)
   - [5.6 Ranks](#56-ranks)
6. [Financials](#6-financials)
   - [6.1 Transactions](#61-transactions)
   - [6.2 Withdrawal Requests](#62-withdrawal-requests)
7. [Content](#7-content)
   - [7.1 Blog](#71-blog)
   - [7.2 Banners](#72-banners)
   - [7.3 Pages](#73-pages)
   - [7.4 Promotions](#74-promotions)
8. [Risk Management](#8-risk-management)
   - [8.1 Risk Groups](#81-risk-groups)
9. [Reports & Analytics](#9-reports--analytics)
   - [9.1 Player Action Logs](#91-player-action-logs)
10. [Platform Settings](#10-platform-settings)
    - [10.1 General](#101-general)
    - [10.2 Countries](#102-countries)
    - [10.3 Languages](#103-languages)
11. [Admin Management](#11-admin-management)
    - [11.1 Admin Users](#111-admin-users)
    - [11.2 Roles & Permissions](#112-roles--permissions)
...
- **First-Time Deposits (FTD)** — Time-series chart and daily table counting players making their *first ever* deposit. This is a core conversion metric — it measures how many registered players actually convert into paying/funded players, distinct from raw registrations.
...
- **Edit**
- **Set Password**
- **Archive** (moves the account to Archived — see [2.3](#23-archived-players))
...
4. Depending on the request:
   ...
   - **Verification needed** → proceed to **KYC** tab or the dedicated [KYC Queue](#22-kyc-queue).
...
- **Document verification workflow:** Opens into the same **Accept / Reject** controls seen on a player's **KYC** tab (see [2.1](#21-all-players)), reviewed per-document (front, back, selfie, proof of address).
...
1. A player is removed from the active pool — typically via **Actions → Archive** on their profile (see [2.1](#21-all-players)), or as the outcome of a self-exclusion/termination process.
...
- **Search & filter:**
  ...
  - **Advanced Filters** — same rule-builder pattern as [All Players](#21-all-players) (Add Rule / Add Group).
...
- **Row-level action:** `View` — opens the same full **player profile** used throughout Players Management, so no functionality is duplicated — VIP Players is purely a curated entry point into the standard player record. The profile's tab structure is broken down in detail in [3.2](#32-vip-player-profile--tab-by-tab-purpose) below, specifically from the lens of managing a high-value account.
- **VIP identity markers on the profile:** Once opened, a VIP player's profile header displays a **VIP** badge alongside a **tier/rank label** (e.g., *Newcomer*) next to the account status and KYC status — indicating the platform tracks VIP progression through ranked tiers, not just a binary VIP flag.
- **Promotion entry point:** Players are added to this view via **Actions → Promote to VIP** on their individual profile (see [2.1](#21-all-players)); there is no separate "add VIP" action inside this module itself — promotion happens from the player record.
...
Opening a **View** on any VIP player leads into the same profile shell used across Players Management, built around a wallet summary (Fiat currency, Real balance, Bonus balance) and nine tabs. For a *VIP* account specifically, each tab carries extra weight because the financial exposure, fraud risk, and service expectations attached to a high-value player are all larger than for a standard player. This section explains what each tab does and — more importantly — **why it matters when the player is a VIP.**
...
- **Shared configuration fields across all bonus types:**
  ...
  - **Games scope** — Include or Exclude mode against Categories, Providers, or specific Games, controlling which bets count toward wagering progress (see also [4.4 Wager Weight](#44-categories), which is a category-level version of the same idea).
...
- **Edit Cashback:** multi-language Name, Slug, Days, Min Threshold, Excluded categories, Excluded providers, Active toggle. The actual **percentage** paid out is not set here — it comes from the player's current rank via the **Cashback %** / **Cashback Max ($)** fields on that rank (see [5.6](#56-ranks)).
...
- Rate is likewise driven by the player's **Rank** — the **Rakeback %** / **Rakeback Max ($)** fields (see [5.6](#56-ranks)), not configured per-cycle here.
...
| **Configurations** | **Wheel Groups** that bind a set of player ranks to a specific wheel of prizes and win chances. Observed groups: *Wheel Group 1 – Newcomer*, *Wheel Group 2 – Beginners* (Traveler/Explorer/Adventurer), *Wheel Group 3 – Super Rank* (Expert/Master/Grandmaster/Captain), *Wheel Group 4 – Mega Rank* (Admiral/Space Whale/Legend/Myth) — i.e., prize quality scales with loyalty tier. |
...
- **Edit Rank fields:**
  ...
  - **Rewards** — Cashback % and Cashback Max ($), plus Rakeback % and Rakeback Max ($) — the actual rates paid out by the Cashback ([5.2](#52-cashback)) and Rakeback ([5.3](#53-rakeback)) cycles for a player at this rank.
...
3. Admin cross-checks rank changes against **Bonus Wheel → Configurations** ([5.4](#54-bonus-wheel)) to make sure the wheel-group-to-rank mapping still makes sense after any rank restructuring.
...
**Core Purpose:** The dedicated approval queue for player withdrawal requests — the operational counterpart to the Dashboard's **Pending Withdrawals** KPI ([1](#1-dashboard-overview)), and the module where an admin actually clears that backlog.
...
![Blog list](images/content/content-blog-list.png)
![Edit Blog Post](images/content/content-blog-edit.png)
...
![Banners list](images/content/content-banners-list.png)
![Edit Banner](images/content/content-banners-edit.png)
...
![Pages list](images/content/content-pages-list.png)
![Edit Page](images/content/content-pages-edit.png)
...
![Promotions list](images/content/content-promotions-list.png)
![Edit Promotion](images/content/content-promotions-edit.png)
...
Risk Management is the platform's bulk-restriction and fraud-mitigation tool. Rather than restricting players one at a time from their individual profile ([2.1](#21-all-players)), it lets admins define a reusable **Risk Group** — a named restriction template — and apply it to many players at once, with the restriction taking effect immediately across every member. It currently contains a single sub-view, **Risk Groups**.

---

### 8.1 Risk Groups

**Core Purpose:** Bulk-manage player restrictions via reusable, named groups, rather than toggling individual Action Controls on every affected player one by one ([2.1](#21-all-players)). The module's own description puts it plainly: *"Add players to a group to bulk-apply restrictions — they take effect immediately on every member."*
...
- **Create Risk Group fields:**
  ...
  - **Restrictions** — five toggles applied to every member immediately: **Can Bet**, **Can Deposit**, **Can Withdraw**, **Bonus Allowed**, **Blocked**. These mirror the individual Action Controls on a player's Dashboard tab ([2.1](#21-all-players)) — the group is effectively a way to set those same switches for many accounts at once. The UI explicitly warns: *"Toggle OFF to restrict."*
- **Search, Advanced Filters, Columns, Saved Views** — consistent with other list modules.

**Standard Operational Workflow:**

1. Risk/compliance identifies a pattern requiring bulk action — e.g., a cluster of accounts linked via [Risk → Identity Links](#32-vip-player-profile--tab-by-tab-purpose) on player profiles, a batch of accounts flagged for a specific fraud pattern, or a jurisdiction-wide restriction.
...
4. Admin adds players to the group (from the group itself once created, and/or via the **Risk Group** field available in [All Players → Advanced Filters](#21-all-players), which can be used both to filter by existing group membership and as the assignment target).
...
![Risk Groups list](images/risk-management/risk-groups-list.png)
![Create Risk Group](images/risk-management/risk-groups-create.png)
...
**Core Purpose:** *"All balance-affecting actions across the platform"* — the definitive, append-only audit log behind every wallet change on the system, used for investigation, reconciliation, and compliance/audit requests.
...
- **Table columns:** ID, Player ID (links to profile), Wallet ID, Username, Entity, Action, Entity ID (the underlying record's own ID in its source table — e.g., the Transaction ID or Bonus ID), Balance Before, and a **View entity** action per row that opens an Entity Details modal with the underlying record. *(Note: in testing, this modal successfully loaded for some entity types and returned "Couldn't load entity" for others — treat it as a convenience shortcut rather than a guaranteed path, and fall back to the entity's own module — e.g., [Financials → Transactions](#61-transactions) — when it fails.)*
...
![Player Action Logs](images/reports-analytics/action-logs-list.png)
...
> **Admin tip:** The KYC/non-KYC withdrawal toggles and the Monthly/Weekly withdrawal limits are two different control layers — the toggles gate a specific frictionless-withdrawal *threshold amount* based on identity verification, while the limits cap total withdrawal *volume* over time regardless of KYC status. Review both together when adjusting withdrawal policy, since either alone gives an incomplete picture of what a player can actually cash out.

![Deposits & Withdrawals settings](images/platform-settings/general-deposits-withdrawals.png)
![Chat Settings](images/platform-settings/general-chat-settings.png)
![Bonus Wheel settings](images/platform-settings/general-bonus-wheel-settings.png)

---

### 10.2 Countries

**Core Purpose:** *"Manage geographic availability and restricted regions"* — the platform-wide geo-compliance control, determining where the platform can legally/operationally accept players.
...
![Countries list](images/platform-settings/countries-list.png)
![Edit Country](images/platform-settings/countries-edit.png)

---

### 10.3 Languages

**Core Purpose:** *"Languages available across the player-facing site and admin"* — the master catalog of every language the platform could support, with an Active flag controlling which are actually live.
...
- 181 languages observed in the catalog — a comprehensive ISO reference list — though in practice, content throughout the panel (Blog, Banners, Pages, Promotions, Leaderboards, Ranks — see [Content](#7-content) and [Engagement](#5-engagement)) is only actively localized into a small subset (English, Korean, Chinese observed), meaning most catalog entries exist as available-but-unused options rather than languages with real translated content.
...
3. This list is purely the *availability* catalog — actually translating content (Blog posts, Page text, Bonus/Leaderboard copy) happens per-record in each respective module's own language tabs, not here.

![Languages list](images/platform-settings/languages-list.png)
...
**Core Purpose:** *"Manage admin users and their roles"* — the master roster of every staff account with access to this panel, and the tool for onboarding, updating, disabling, and securing those accounts.
...
- **Add Admin User fields:** First Name, Last Name, Email, Password (minimum 8 characters, mixed case, a number, and a symbol), and **Roles** — a multi-select chip picker against the roles defined in [Roles & Permissions](#112-roles--permissions) (`Admin`, `Super Admin`, `Moderator`, `User` observed).
...
![Admin Users list](images/admin-management/admin-users-list.png)
![Edit Admin User](images/admin-management/admin-users-edit.png)

---

### 11.2 Roles & Permissions

**Core Purpose:** *"Manage roles and the pages and tabs each role can access"* — the actual access-control definitions referenced by every account in [Admin Users](#111-admin-users). This is the single source of truth for what "Admin," "Super Admin," "Moderator," or any custom role is actually allowed to do inside the panel.
...
4. Once a role's checkboxes are set as intended, admin clicks **Save Changes** — every [Admin User](#111-admin-users) currently assigned that role is affected immediately, platform-wide.
...
> **Admin tip:** Because permission changes apply immediately to every user holding that role, editing a *role* (rather than an individual user's access) is a blast-radius action — a single Save Changes here can expand or restrict what dozens of staff can do at once. Prefer creating a new, narrowly-scoped role over broadening an existing shared role like Admin or Moderator when only one person's access actually needs to change.

![Roles & Permissions list](images/admin-management/roles-list.png)
![Edit Role — expanded permission tree](images/admin-management/roles-edit-permissions.png)

---

*This concludes the modular documentation of the Asdfbet Admin Panel, covering all eleven top-level sections of the sidebar: Dashboard, Players Management, VIP Management, Games, Engagement, Financials, Content, Risk Management, Reports & Analytics, Platform Settings, and Admin Management.*
